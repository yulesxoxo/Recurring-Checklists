				...
				{#if editingChecklist}
					<section class="rounded-container border border-surface-800 bg-surface-900 p-5 shadow-sm">
						<form class="flex flex-col gap-5" onsubmit={(event) => event.preventDefault()}>
							...
							>
								{#each editingChecklist.sections as section, sectionIndex (section.id)}
									<Accordion.Item
									...
									>
										...
										>
											...
											<div class="flex flex-wrap justify-end gap-2">
												...
												<button
													...
													disabled={sectionIndex === editingChecklist.sections.length - 1}
													onclick={() => moveSection(section.id, 1)}
												>
										...
										<Accordion.ItemContent class="p-4">
											...
											<div
												class={`grid gap-3 ${
													section.schedule.frequency === 'minutely' ? '' : 'sm:grid-cols-2'
												}`}
											>
												<label class="label">
												...
												{#if section.schedule.frequency !== 'minutely'}
													<div class="label">
														...
														<span class="text-xs text-surface-400">
															Local {scheduleInputTime(
																{ ...section.schedule, timeMode: 'local' },
																now
															)}
